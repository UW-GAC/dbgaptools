...
test_that("Missing ID columns are detected", {
  expect_error(check_sattr(sattr_ds_file, sampleID_col = "mysample"),
               "Please check that ds contains column for sample-level ID", fixed = TRUE)
})

test_that("Non-standard ID column names are detected", {
  ds.rev <- read_ds_file(sattr_ds_file)
  ...
  expect_warning(check_sattr(ds = ds.rev.fn, sampleID_col = "mysample"), str, fixed = TRUE)
  unlink(ds.rev.fn)
})
...
test_that("Extra samples are detected", {
  ds <- read_ds_file(sattr_ds_file)
  samp_exp <- ds[, 1]
  samp_exp_less <- samp_exp[-c(3:4)]
  out <- check_sattr(sattr_ds_file, samp_exp = samp_exp_less)
...
test_that("Missing samples are detected", {
  ...
  out <- check_sattr(sattr_ds_file, samp_exp = samp_exp_more)
...
test_that("Missing and required TOPMed variables are detected", {
  out <- check_sattr(sattr_ds_file, topmed = TRUE)
